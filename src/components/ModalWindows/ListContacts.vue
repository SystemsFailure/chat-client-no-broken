<template>
    <div class="main-contacts-window">
        <div class="lds-ripple" v-if="localLoader"><div></div><div></div></div>
        <div class="inner">
            <div class="close-btn">
                <img src="@/assets/svgassets/icons8-удалить.svg" alt="" srcset="" @click="this.$emit('closeListContactsWindowFunction')">
            </div>
            <div class="list-contacts">
                <div class="item" v-for="it in listContacts" :key="it.id">
...
export default {
    ...
    async mounted() {
        ...
        {
            await UserApi.getAllChats(this.$store.state.USERID).then(array => {
                this.listContacts = array
